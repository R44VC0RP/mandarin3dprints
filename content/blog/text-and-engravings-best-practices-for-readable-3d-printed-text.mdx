...
Text is one of the trickiest things to get right in FDM printing. The process can struggle with fine letterforms, but careful sizing, orientation, and sample prints can improve readability.
...
Upward-facing text can print cleanly, but its strokes are resolved in the XY plane by extrusion width and toolpaths, not layer height alone. Layer height controls the steps in relief or engraving depth. Compare both in the [layer-height guide](/blog/3d-print-resolution-guide-what-layer-height-should-you-choose).
...
Text on sloped surfaces can show stair-stepping, with the result depending on angle and layer height. Do not assume steeper surfaces always show more stepping. Embossed text may also create overhangs; check the sliced preview and print a sample.
...
Inspect the sliced preview for missing strokes and filled-in counters, then check a physical sample at the intended viewing distance. A detailed-looking model on screen does not establish that small letters will remain readable in plastic.
...
For labels that must be read reliably, test the final color and finishing method as well as the geometry. Paint fill, lighting, and viewing distance can matter as much as character size.
