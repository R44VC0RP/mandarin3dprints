...
Printing the thread axis vertically can improve thread shape, but it is not a non-negotiable rule. Axial loads can then pull across layer bonds. Compare thread quality with the load path using the [orientation guide](/blog/choosing-the-right-print-orientation-for-your-model).
...
Layer height affects how finely the thread profile is resolved. Use several layers per pitch and stay within the nozzle profile's supported range. The following examples are starting points, not a guarantee of fit; 0.4mm layers may require a larger nozzle than the common 0.4mm diameter.
...
Nominal thread geometry may bind because of dimensional variation and surface texture. Use the [measurement and fit guide](/blog/how-to-measure-your-model-for-perfect-fit-every-time), print a short sample, and adjust the mating feature rather than scaling every dimension of the part.
...
PETG is a useful candidate for tapped threads, but test the pilot hole and surrounding wall thickness before tapping the final part.
...
**PETG:** A practical starting point for many threaded parts because it is less brittle than standard PLA. Thread engagement, wall thickness, and installation torque still need testing.
...
Test the connection using the actual fastener and intended assembly method before printing a full batch. For inserts, follow the manufacturer's hole and installation specifications, then check that the insert resists rotation and pull-out under the intended loads.
