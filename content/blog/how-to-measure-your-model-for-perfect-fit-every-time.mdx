...
Poor measurements can make an otherwise useful model fail to fit. A ruler may not resolve a small mating feature, and even careful measurements need allowance for manufacturing variation. Here's how to measure and verify the resulting fit.
...
3D printing isn't injection molding. Every printed part has dimensional variation. Around +/-0.2mm can be a useful starting estimate for a well-tuned FDM printer, but actual accuracy depends on the machine, material, geometry, and settings. Measure test prints rather than treating that figure as a guaranteed tolerance. The [design-modification guide](/blog/why-your-3d-model-might-need-modifications-before-printing) distinguishes tolerance from clearance and shows a diametral-versus-radial example.
...
**Tight or press fit:** A press fit is an interference fit; a positive nominal clearance does not guarantee one. Print a range of mating samples and measure the result before selecting dimensions. Thin walls may split during insertion.

**Snug sliding-fit candidate:** Try a 0.2mm total difference between the mating dimensions, then test. This is not a promised fit, and for a centered round peg it means a 0.1mm radial gap.

**Looser sliding-fit candidate:** Compare total dimensional differences of 0.3-0.4mm. Actual friction depends on texture, shape, material, and alignment, not just nominal clearance.
