...
FDM prints build on previous layers, so outward-sloping features need enough support from the layer below. Around 45 degrees from vertical is a starting guideline, not a fixed machine limit. Use the [support-free design guide](/blog/how-to-design-parts-that-dont-need-supports) and test the actual material and profile.
...
Thin walls can disappear during slicing or be too fragile for handling. The [wall-thickness guide](/blog/wall-thickness-guide-minimum-and-optimal-measurements) explains extrusion width, variable-width slicing, and why the following values are prototype starting points rather than absolute limits.
