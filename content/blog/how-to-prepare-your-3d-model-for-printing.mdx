...
Check which formats your slicer or chosen print provider accepts. Open the exported file and verify its dimensions before printing.
...
- **Print providers** may offer file repair; confirm whether it is included and approve any geometry changes
...
With a common 0.4mm nozzle, 1.2mm or 1.6mm walls are useful starting points. Check the actual extrusion width and sliced toolpaths rather than relying on nozzle diameter alone.
...
Run through this list before printing or sharing the file:
...
Before committing material or approving a print:

1. Open the exported file and compare its dimensions with the intended size
2. Slice it using the target printer and material profile
3. Inspect thin walls, unsupported sections, and missing features in the layer preview

A clean file reduces avoidable failures, but fit-critical features still benefit from a small test print.
...
If you need someone else to print the model, use the [local printing guide](/#local-printing) and ask prospective providers how they handle file review and design changes.
